...
class SensorService {
  ...
  void start() {
    _accelerometer = accelerometerEventStream(samplingPeriod: const Duration(milliseconds: 100)).listen((event) {
      _lastAcceleration = event;
      _pushIfReady();
    });
    _gyroscope = gyroscopeEventStream(samplingPeriod: const Duration(milliseconds: 100)).listen((event) {
  ...
  void _pushIfReady() {
    ...
    if (_buffer.length == 20) _windows.add(List.unmodifiable(_buffer));
  }
